...
if [ -n "$PHPSTORM_JDK" -a -x "$PHPSTORM_JDK/bin/java" ]; then
  JDK="$PHPSTORM_JDK"
elif [ -s "$HOME/.PhpStorm2016.2/config/phpstorm.jdk" ]; then
  JDK=`$CAT $HOME/.PhpStorm2016.2/config/phpstorm.jdk`
  if [ ! -d $JDK ]; then
    JDK=$IDE_HOME/$JDK
  fi
...
elif [ -r "$HOME/.PhpStorm2016.2/phpstorm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.PhpStorm2016.2/phpstorm$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=PhpStorm2016.2"
  fi
fi
...
LD_LIBRARY_PATH="$IDE_BIN_HOME:$LD_LIBRARY_PATH" "$JAVA_BIN" \
  ...
  -Djb.restart.code=88 -Didea.paths.selector=PhpStorm2016.2 \
...
RESTARTER="$HOME/.PhpStorm2016.2/system/restart/restarter.sh"
if [ -x "$RESTARTER" ]; then
  "$RESTARTER"
  "$RM" -f "$RESTARTER"
fi
